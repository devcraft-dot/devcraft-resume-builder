...
function FileBtn({ url, label }: { url: string; label: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5">
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="inline-flex items-center gap-1 text-xs font-medium px-2 py-1 rounded bg-green-50 text-green-700 hover:bg-green-100 transition"
        title={`Edit ${label} in Google Docs`}
      >
          ...
          <tbody className="divide-y divide-gray-100">
            {rows.map((row) => (
              <tr key={row.id} className="hover:bg-gray-50 transition">
                ...
                {/* Resume file */}
                <td className="px-3 py-2.5">
                  <FileBtn url={row.resume_drive_url} label="Resume" />
                </td>

                {/* Q&A file */}
                <td className="px-3 py-2.5">
                  <FileBtn url={row.questions_drive_url} label="Q&A" />
                </td>

                {/* JD file */}
                <td className="px-3 py-2.5">
                  <FileBtn url={row.jd_drive_url} label="JD" />
                </td>
